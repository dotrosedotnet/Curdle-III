...
grid_try   = '│' + '│'.join(( ' * LETTER_COUNT) + '│'
grid_open  = '┌' + '┬'.join('─' * LETTER_COUNT) + '┐'
grid_break = '├' + '┼'.join('─' * LETTER_COUNT) + '┤'
grid_close = '└' + '┴'.join('─' * LETTER_COUNT) + '┘'
...
empty_grid = [
    '┌' + '┬'.join('─' for _ in range(LETTER_COUNT)) + '┐',
]
...
def a_guess(y,x,l):
    ...
    while True:
        ...
        if str.isalpha(letter) and len(this_guess) < l:
        ...
        if (
            len(guess_list) == LETTER_COUNT
            and this_guess.upper() in words
            and letter in ['\n', '\r', 'KEY_ENTER']
        ):
                return this_guess.upper()
...
KEYBOARD_FB = {l: {"score": 0, "line": 0} for l in abcs}
...
def check_guess(y,x):
    ...
    if this_guess == word:
        ...
        scores = {}
        for i, l in enumerate(this_guess):
            scores[i] = {"letter": l, "correct": 0, "present": 0, "absent": 0, "marked": 0}
...
def guess_conveyor(y,x):
    success = False
    guesses = 0
    while guesses < TRY_COUNT and not success:
...
def grade_word(secret_word, guess):
    """Grades guess for correctness against secret word
    Grade per letter:
        - Absent
        - Present
        - Correct       # enum module? dataclass? tuples?
    Grading logic:
        - Correct marking takes precedent over Present
        - Present + Correct instances !> letter instances in secret word
            - (present letters which exceed instance count in secret word will be marked as absent)
    """
    pass
# [('s', 'absent''), ('u', 'present'), ...]
